...
## 2026-08-09 — v2.39.0: the Work tab now shows the real interview list

- **Reported:** "if I mark them off on my personal tab, they should sync to the quarterly interviews tab. I don't think that's happening." Correct, and the docs were lying: this file and the guide both claimed a bidirectional sync. There was none. The Work tab's `Interview HC (…)` rows are ordinary quarterly behaviors seeded by the Stake President / Counselor templates (`seedTemplates.ts`), writing to `steward_entries`; the interviews page writes `steward_interviews`. The only real link was `MyInterviewsCard`, deleted in v2.18.2 (2026-05-10) as "duplication" on the reasoning that the template placeholders covered it — they never did, they just used similar words.
- **Fix chosen (Scott's call, offered against a per-behavior link mapping): one source of truth.** The Work tab's "This Quarter" section now renders the caller's assigned interviews directly out of `steward_interviews`. Added a `VirtualRow` concept to `PeriodChecklist` (non-behavior rows rendered in the same list, counted in the "n of m done" header); `src/app/page.tsx` builds `myInterviewRows` from `useInterviews(year, uid, enabled)` filtered to `assigned_to_user_id = me` and the quarter currently displayed, so the quarter stepper moves the interview list too. Toggling calls the same `toggleComplete` the interviews page uses. The two views cannot disagree because there is only one record.
- Rejected the alternative (a `linked_interviewee` column on behaviors with two-way sync) because it needs a per-behavior mapping redone whenever assignments change, and leaves anyone assigned-but-unmapped invisible on the Work tab.
- Rows show interviewee + calling, and once done show who marked it and when (`last_updated_by`). `pickCompletionDate` moved from the interviews page into `useInterviews` so both surfaces stamp dates identically. `useInterviews` gained an `enabled` flag so non-presidency users never query.
- **Bug found while wiring attribution:** `useInterviews` read `steward_user_profiles` directly, but that table's SELECT policy is self-or-admin. Both counselors are non-admin, so their roster came back with exactly 1 row — the interviews grid has been showing "Unknown" for every assignee for them, and attribution would have been nameless. Added `steward_presidency_members()` (SECURITY DEFINER, gated on the existing `steward_caller_can_manage_interviews()`), migration `supabase-migration-v2.39.0-presidency-members.sql`, applied. Verified as the first counselor: direct read = 1 profile, RPC = 5.
- Placeholder cleanup: when a leader has real assigned interviews AND quarterly behaviors named `Interview …`, the Work tab offers a one-tap archive (archive, not delete — history kept). Dismissible.
- **Verified** against live data in rolled-back transactions as the (non-admin) first counselor: 32 interviews visible, 10 assigned in Q3, roster resolves to 5 names, and a completion write stamps `last_updated_by` correctly. Nothing persisted (Q2/Q3 completion counts unchanged at 13/7). `tsc --noEmit` and `npm run build` clean; eslint clean on the touched app files.
- **Follow-up not done:** `seedTemplates.ts` still seeds the placeholder `Interview HC (…)` behaviors into the three presidency templates, so a newly onboarded stake gets the same duplication. Left alone because it changes what every other stake's onboarding produces — Scott's call.
- State left in: v2.39.0, pushed to main, migration applied.
